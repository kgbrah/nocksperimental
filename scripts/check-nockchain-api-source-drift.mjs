...
const tracePath = "src/lib/nockchain-api-source-trace.ts";
const traceFactory = "createNockchainApiSourceTrace";
const userAgent = "nocksperimental-api-source-drift-check";
const compareFields = [
  "upstreamCommit",
  "sourceAnchorId",
  "sourceSha256",
...
function createImpact(trace) {
  const targetSurfaces = trace.sourceDriftCheck?.targetSurfaces ?? ["nockchainApiSourceTrace"];
...
function printTextReport(report) {
  console.log(`Nockchain API source drift: ${report.status}`);
  console.log(
