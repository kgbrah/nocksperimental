...
const tracePath = "src/lib/nockchain-pma-source-trace.ts";
const traceFactory = "createNockchainPmaSourceTrace";
const userAgent = "nocksperimental-pma-source-drift-check";
const compareFields = [
  "upstreamCommit",
  "sourceAnchorId",
  "sourceSha256",
...
function createDriftReport(trace, snapshot) {
  ...
  return {
    ...
    interpretation:
      "Compares Nocksperimental's commit-pinned Nockchain PMA, snapshot, and event-log source anchors against current upstream master, including source hashes, byte counts, and required durability symbols.",
    snapshot: {
    ...
    nextActions: [
      "Refresh src/lib/nockchain-pma-source-trace.ts before using PMA, snapshot, or event-log source anchors as current state-jam or bootstrap evidence.",
      "Review changed PMA, snapshot, and event-log files against the Rust source guide before updating state-jam, fakenet, or Launch Evidence receipts.",
...
function createImpact(trace) {
  const targetSurfaces = trace.sourceDriftCheck?.targetSurfaces ?? ["nockchainPmaSourceTrace"];

  return {
    impactedRepos: ["nockchain/nockchain"],
    sourceAuthorities: ["canonical-nockchain-pma-rust", "canonical-nockchain-snapshot-rust"],
    reviewClassIds: ["rust-workspace", "pma-state-jam"],
    sourceRouteIds: [
...
function printTextReport(report) {
  console.log(`Nockchain PMA source drift: ${report.status}`);
  console.log(
