...
const tracePath = "src/lib/nockchain-mining-source-trace.ts";
const traceFactory = "createNockchainMiningSourceTrace";
const userAgent = "nocksperimental-mining-source-drift-check";
const compareFields = [
  "upstreamCommit",
  "sourceAnchorId",
  "sourceSha256",
...
function createImpact(trace) {
  const targetSurfaces = trace.sourceDriftCheck?.targetSurfaces ?? ["nockchainMiningSourceTrace"];
...
function printTextReport(report) {
  console.log(`Nockchain mining source drift: ${report.status}`);
  console.log(
