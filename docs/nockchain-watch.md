...
- **Updated:** 2026-07-20 — Six new crates in the upstream workspace: `hatch` (native Rust Hoon parser), `honk` (native Rust Hoon→Nock compiler), `honk-tools`, `honc-cold-138`, `kernels/roswell`, and `roswell` (new ZK-capable kernel binary using `zkvm-jetpack`). Front #1 Bridge Withdrawals: withdrawal protocol is now implementation-wired on both Nock kernel and Rust runtime sides (no longer unpublished). Multiple source files drifted across bridge, wallet, mining, PMA, API, and sync/gossip domains. Roadmap scraper blocked (403 on nockchain.org/roadmap) and GitHub Issues API blocked (403) — public site milestone state unverifiable this cycle; PR radar check skipped. Zorp org: all 10 previously-tracked repos now show 0 public repos (org may have gone private).
- **Previous update:** 2026-06-10 — Front #2 (AI Compute Market / Fork A) flipped to IMPLEMENTATION OPEN: PR #124 (AI-PoW matmul, `ai-pow-miner` + `ai-pow-zk`) is now a full open PR on `nockchain/nockchain`. See PR-radar #124.
- **Sources:** nockchain.org/roadmap (33 milestones, **blocked this cycle — 403**), docs.nockchain.org (39
...
| --- | --- | --- | --- | --- | --- |
| 1 | **Bridge withdrawals** | **IMPLEMENTATION WIRED** (as of 2026-07-20; was "mechanics unpublished") | Two-way Nockchain↔Base transfers | Our xchain fixtures, bridge reconciliation evidence, MessageInbox/tNOCK mirror. Official bridge = 3-of-5 ecosystem multisig (Zorp×2, SWPS, Nockbox, Lambda). **As of 2026-07-20:** Nock kernel withdrawal tx detection wired, settlement arm implemented, Base-side proposal arm implemented; Rust runtime now wires deposit+withdrawal assembly, signing, submission loops. `bridge-dev/tests/scenarios.rs` grew +18KB. `bridge-withdrawals.md` updated. Not yet COMPLETED — watch for roadmap flip and a dedicated writings post. | Roadmap flip to COMPLETED; a dedicated writings post; drift check |
...
| 3a | **Native Hoon compiler + Roswell kernel** | **IN PROGRESS** (as of 2026-07-20) | Rust-native Hoon→Nock compilation path (`hatch` parser + `honk` compiler), byte-for-byte parity with `hoonc`; `roswell` kernel binary with `zkvm-jetpack` dependency (ZK-capable kernel runner) | Build tooling: Bazel native Hoon rules wired through `honk` CLI. **Lab/evidence**: a native compiler enables verified, reproducible compilation — future invariant packs could prove the Hoon→Nock step. **ZKVM path**: `roswell` using `zkvm-jetpack` likely part of the ZKVM kernel track (Front #3). Honk-tools adds `jam-diff` / `extract-hoonc-octs-type` diagnostics. | New writings or changelog post announcing native compiler; `honk` GitHub release/CI going green; further `roswell` kernel documentation |
...
- **2026-07-20**: `nockchain.org/roadmap` returning 403 — automated scraper blocked. Milestone state unverifiable from automation this cycle; manual check required. GitHub Issues API also returning 403 for the PR-radar script.
- **2026-07-20**: Zorp org (`zorp-corp/`) shows 0 public repos; previously tracked 10 repos now appear private or moved. The `check:zorp-org-drift` script will continue reporting all 10 as "extra local" until a human confirms the org status.
- **2026-07-20**: 6 new crates in the upstream `nockchain/nockchain` workspace (hatch, honk, honk-tools, honc-cold-138, kernels/roswell, roswell) not yet reflected in our pinned manifests — baseline must be updated after human review of the new crates.
