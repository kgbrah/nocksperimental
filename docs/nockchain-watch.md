...
- **Updated:** 2026-06-22 — New `roswell` binary + `kernels-roswell` kernel crates added to workspace (`zkvm-jetpack` dependency; commands: `prove-puzzle`, `make-proof-snapshot`, `assemble-proof-stream`, `assemble-proof-continuation`, `check-proof`, plus verifier/bench suites). Likely the PoUW puzzle proving harness and/or early ZKVM proof-generation infrastructure; watch for tie-in with PR #124 (Fork A) or the ZKVM timeline (Front #3). Two Tier 1 READMEs changed upstream (`crates/nockapp` — hoonc docs + MINIMAL\_LOG\_FORMAT; `crates/nockchain-wallet` — content updates). All existing Cargo.toml files bumped ~26 bytes each (uniform workspace metadata change, not semantically significant). Roadmap drift check and PR radar both 403-blocked this cycle; roadmap baseline not re-pinned; Front statuses below are carried from 2026-06-10.
- **Previous (2026-06-10):** Front #2 (AI Compute Market / Fork A) flipped to IMPLEMENTATION OPEN: PR #124 (AI-PoW matmul, `ai-pow-miner` + `ai-pow-zk`) is now a full open PR on `nockchain/nockchain`. See PR-radar #124.
...
| 3 | **Full Nock ZKVM** | PLANNED Q3 2026 ("toward end of year" per Phase 2 post); new `roswell` proving binary (+ `kernels-roswell`) in workspace as of 2026-06-22 — first dedicated proof-gen CLI using `zkvm-jetpack` | Performant general-purpose ZK proofs | Prerequisite for `zkp` opcode and next PoUW puzzle; its completion **triggers the 80/20 coinbase reversion** | `roswell` proof commands expanding; docs/roadmap flip; `zkvm-jetpack` API changes |
...
## Automated check blind spots (last recorded 2026-06-22)

- **GitBook site-index → 403**: `check:nockchain-roadmap-drift` cannot fetch `https://docs.nockchain.org/~gitbook/site-index` from the CI environment. Roadmap/writings/docs-page drift is not checkable until this resolves; baseline not re-pinned this cycle.
- **GitHub unauthenticated API → 403**: PR radar (`check:nockchain-pr-radar-drift`), most source-anchor checks, bridge/wallet source checks, and Zorp-org check all fail. PRs (including #124 status) and source diffs are blind.
- **Release assets → 404**: `check:nockchain-release-assets-drift` returns 404 — zero published GitHub releases remain a known state; no action needed unless releases reappear.
