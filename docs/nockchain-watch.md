...
- **Updated:** 2026-06-15 — Two new crates merged to `nockchain/nockchain` master: `crates/roswell` + `crates/kernels/roswell` (Roswell ZKVM proof-pipeline harness — test runner, proof streaming, assembly, verification; uses `zkvm-jetpack`). `crates/kernels/bridge` renamed to `kernels-open-bridge`. Widespread manifest churn (36/36 existing crates + 2 new). `nockchain.org/writings` now returns 403 to the drift script's UA — roadmap surface check blocked this run; docs drift in-sync. PR radar + source-anchor checks blocked by GitHub API rate limit (no token in CI). No new `activation_height` confirmed (docs page blocked by network allowlist). See alignment doc for app impact.
...
| --- | --- | --- | --- | --- | --- |
| 1 | **Bridge withdrawals** | **CURRENT** (Q2 2026; slipped from Q1) — `crates/kernels/bridge` package renamed `kernels-open-bridge` (2026-06-15); withdrawal mechanics still unpublished | Two-way Nockchain↔Base transfers | Our xchain fixtures, bridge reconciliation evidence, MessageInbox/tNOCK mirror. Official bridge = 3-of-5 ecosystem multisig (Zorp×2, SWPS, Nockbox, Lambda) over a deterministic event-mirroring state machine — withdrawal mechanics still unpublished | Roadmap flip to COMPLETED; a dedicated writings post; drift check |
...
| 3 | **Full Nock ZKVM** | PLANNED Q3 2026 ("toward end of year" per Phase 2 post) — **Roswell harness merged (2026-06-15)**: new `crates/roswell` + `crates/kernels/roswell` provide a productionized proof-pipeline CLI (prove-puzzle, proof-streaming via make-proof-stream-window/assemble-proof-stream/assemble-proof-continuation, check-proof, full test/bench suite) | Performant general-purpose ZK proofs | Prerequisite for `zkp` opcode and next PoUW puzzle; its completion **triggers the 80/20 coinbase reversion** | Repo activity; roadmap flip |
...
## Known automation gaps (tracked)

- **nockchain.org/writings returns 403** (observed 2026-06-15): the roadmap drift script's user-agent is blocked by the site. The writings surface cannot be auto-compared. Monitor manually at nockchain.org/writings until the check is re-enabled (e.g. with a rotating UA or a cached proxy).
- **GitHub REST API rate-limited** (no auth token in CI): `pr-radar`, `bridge-source`, `wallet-source`, `zorp-org`, `pma-source`, `mining-source`, `runtime-safety-source`, `api-source`, `nockup-source`, `testkit-e2e-source`, `sync-gossip-source` all fail with 403. These checks need a `GITHUB_TOKEN` env var to function reliably. Release-assets also 403 (may overlap with the zero-releases known issue).
