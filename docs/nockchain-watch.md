...
- **Updated:** 2026-06-29 — New `roswell` ZK proving binary + `kernels-roswell` kernel landed on upstream master (workspace grew 36→38 members); two Tier 1 READMEs updated on master (`crates/nockapp/README.md`, `crates/nockchain-wallet/README.md`); multiple crate Cargo.toml files drifted (bridge, bridge-dev, others — likely dep bumps); nockchain.org/roadmap + docs.nockchain.org returned HTTP 403 this run (roadmap milestone state unverifiable — manual spot-check needed); GitHub API 403 blocked pr-radar — PR #124 (AI Compute Market) merge status **unknown** as of this run.
...
| 3 | **Full Nock ZKVM** | PLANNED Q3 2026 ("toward end of year" per Phase 2 post) | Performant general-purpose ZK proofs | Prerequisite for `zkp` opcode and next PoUW puzzle; its completion **triggers the 80/20 coinbase reversion** | Repo activity; roadmap flip. **2026-06-29**: New `roswell` binary + `kernels-roswell` kernel landed on master — CLI for ZK puzzle proving, proof-stream assembly, and bench/test suites (wallet, bridge, crypto, verifier); depends on `zkvm-jetpack`. Possible infrastructure for ZKVM proving pipeline or PoUW puzzle mechanism — not a completion signal but notable forward movement |
...
- **2026-06-29 monitoring run**: `nockchain.org/roadmap` and `docs.nockchain.org`
  returned HTTP 403 from the automated monitoring container — roadmap milestone
  statuses and doc page hashes could not be verified this week. GitHub API also
  returned 403 for pr-radar, bridge/mining/PMA source checks. Only raw GitHub
  content fetches (cargo workspace, docs README hashes) succeeded. Next run or
  manual spot-check should confirm whether Bridge Withdrawals flipped to COMPLETED
  and whether PR #124 (AI Compute Market) has merged.
