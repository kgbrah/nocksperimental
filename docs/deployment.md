...
Nocksperimental deploys to Cloudflare Workers with the OpenNext Cloudflare adapter. The production Worker is named `nocksperimental` and is bound to the custom domain `nocksperimental.com` in `wrangler.jsonc`. Persisted fakenet evidence receipts use the `NOCKS_FAKENET_RECEIPTS` Workers KV binding. Persisted VESL evidence receipts use the `NOCKS_VESL_RECEIPTS` Workers KV binding.
