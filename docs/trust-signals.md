...
It resolves the artifact's `issuerKeyId` against the published SPKI (retired keys included, so historical badges keep verifying), recomputes the payload digest, and checks the Ed25519 signature; exit code is `0` only when verified. For production keys signed with `NOCKS_BADGE_ISSUER_SIGNING_SEED`, pin the exported registry via `--keys <registry.json>`. Committed badge revocations (`badgeRevocations` in `src/data/trust-signals.json`, which the verifier already reads) **are** checked offline and fail the badge closed (`verified: false`, `revoked: true`, with `revokedAt`/`replacementBadgeId` surfaced) so the portable verdict matches the host — the signature is still reported cryptographically valid, since revocation is an independent committed fact. Only live freshness/staleness stays out-of-band (the hosted registry/API) and is intentionally **not** checked offline.
